...
Next, on the left-hand side, click `SSH and GPG keys`. Then, click the green button in the top right corner that says `New SSH Key`. Name your key something that is descriptive enough for you to remember where it came from. Leave the `Key Type` to the default key type (`Authentication Key`). Leave this window open while you do the next steps. 
